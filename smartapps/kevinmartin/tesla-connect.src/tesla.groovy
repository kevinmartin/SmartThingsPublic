 ...
 *  Tesla Connect
 *
 *  Copyright 2020 Kevin Martin
 *  Modified from https://github.com/trentfoley/SmartThingsPublic
 *
...
def loginToTesla() {
	...
	return dynamicPage(
		...
		nextPage: "selectVehicles",
		uninstall: showUninstall
	) {
...
def getChildNamespace() { "kevinmartin" }
	...
	if (selectedVehicles) {
		selectedVehicles.each { dni ->
			...
			if(!d) {
				...
				device = addChildDevice("kevinmartin", "Tesla", dni, null, [name:"Tesla ${dni}", label: vehicleName])
